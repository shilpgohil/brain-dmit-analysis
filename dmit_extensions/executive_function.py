from typing import Dict, Any
from .base import DMITExtensionBase

class ExecutiveFunctionExtension(DMITExtensionBase):
    """
    Extension for analyzing Executive Function abilities and cognitive control from fingerprint features.
    Uses comprehensive DMIT scientific correlations between fingerprint patterns and executive functioning.
    """
    def analyze(self, features: Dict[str, Any]) -> Dict[str, Any]:
        # Extract comprehensive fingerprint features for executive function analysis
        # Ridge count and density features
        # tfrc already extracted above
        ridge_density = features.get('ridge_density', 0.0)
        tfrc = features.get('tfrc', 0)  # Total Fingerprint Ridge Count
        ridge_continuity = features.get('ridge_continuity', 0.0)
        ridge_uniformity = features.get('ridge_uniformity', 0.0)
        ridge_thickness = features.get('mean_ridge_thickness', 0.0)
        ridge_orientation = features.get('mean_ridge_orientation', 0.0)
        ridge_curvature = features.get('mean_ridge_curvature', 0.0)
        
        # Pattern analysis features
        pattern_type = features.get('pattern_type', 'loop')
        pattern_symmetry = features.get('pattern_symmetry', 0.0)
        pattern_regularity = features.get('pattern_regularity', 0.0)
        whorl_complexity = features.get('whorl_complexity', 0.0)
        
        # Fractal and complexity features
        box_counting_dimension = features.get('box_counting_dimension', 1.5)
        correlation_dimension = features.get('correlation_dimension', 1.5)
        information_dimension = features.get('information_dimension', 1.5)
        lacunarity = features.get('lacunarity', 0.0)
        fractal_complexity = features.get('fractal_complexity', 0.0)
        
        # Graph and network features
        graph_density = features.get('graph_density', 0.0)
        clustering_coefficient = features.get('clustering_coefficient', 0.0)
        spectral_radius = features.get('spectral_radius', 0.0)
        betweenness_centrality = features.get('betweenness_centrality', 0.0)
        modularity = features.get('modularity', 0.0)
        community_cohesion = features.get('community_cohesion', 0.0)
        
        # Topological features
        euler_characteristic = features.get('euler_characteristic', 0)
        topological_complexity = features.get('topological_complexity', 0.0)
        h1_num_features = features.get('h1_num_features', 0)  # Loop/hole count
        betti_0 = features.get('betti_0', 0)  # Connected components
        betti_1 = features.get('betti_1', 0)  # Loops/holes
        
        # Statistical features
        entropy = features.get('entropy', 0.0)
        std_intensity = features.get('std_intensity', 0.0)
        minutiae_density = features.get('minutiae_density', 0.0)
        minutiae_count = features.get('minutiae_count', 0)
        
        # Spectral features
        spectral_energy = features.get('spectral_energy', 0.0)
        spectral_entropy = features.get('spectral_entropy', 0.0)
        spectral_centroid = features.get('spectral_centroid', 0.0)
        spectral_bandwidth = features.get('spectral_bandwidth', 0.0)
        spectral_rolloff = features.get('spectral_rolloff', 0.0)
        
        # Micro-features
        pore_density = features.get('pore_density', 0.0)
        pore_count = features.get('pore_count', 0)
        
        # Calculate executive function abilities using comprehensive DMIT scientific correlations
        
        # 1. Working Memory Analysis (DMIT Principle: High information dimension + entropy = working memory)
        working_memory = self._calculate_working_memory(information_dimension, entropy, 
                                                      pattern_symmetry, spectral_entropy)
        
        # 2. Cognitive Flexibility Analysis (DMIT Principle: High ridge density + clustering coefficient = cognitive flexibility)
        cognitive_flexibility = self._calculate_cognitive_flexibility(ridge_density, clustering_coefficient, 
                                                                    modularity, ridge_thickness)
        
        # 3. Inhibitory Control Analysis (DMIT Principle: High correlation dimension + graph density = inhibitory control)
        inhibitory_control = self._calculate_inhibitory_control(correlation_dimension, graph_density, 
                                                              betweenness_centrality, information_dimension)
        
        # 4. Planning Analysis (DMIT Principle: High community cohesion + spectral radius = planning)
        planning = self._calculate_planning(community_cohesion, spectral_radius, 
                                          topological_complexity, euler_characteristic)
        
        # 5. Problem Solving Analysis (DMIT Principle: High ridge count + fractal dimension = problem solving)
        problem_solving = self._calculate_problem_solving(tfrc, box_counting_dimension, 
                                                        h1_num_features, betti_1)
        
        # 6. Attention Control Analysis (DMIT Principle: High pattern regularity + low lacunarity = attention control)
        attention_control = self._calculate_attention_control(pattern_regularity, lacunarity, 
                                                            ridge_continuity, std_intensity)
        
        # 7. Decision Making Analysis (DMIT Principle: High ridge uniformity + pattern type = decision making)
        decision_making = self._calculate_decision_making(ridge_uniformity, pattern_type, 
                                                        ridge_curvature, spectral_energy)
        
        # 8. Task Switching Analysis (DMIT Principle: High spectral features + graph complexity = task switching)
        task_switching = self._calculate_task_switching(spectral_centroid, spectral_rolloff, 
                                                      graph_density, topological_complexity)
        
        # Calculate overall executive function score
        executive_function_score = (
            working_memory * 0.20 +                  # Working memory is fundamental
            cognitive_flexibility * 0.18 +           # Cognitive flexibility is crucial
            inhibitory_control * 0.15 +              # Inhibitory control is important
            planning * 0.15 +                        # Planning is essential
            problem_solving * 0.12 +                 # Problem solving
            attention_control * 0.10 +               # Attention control
            decision_making * 0.07 +                 # Decision making
            task_switching * 0.03                    # Task switching
        )
        
        # Normalize to 0-1 range
        executive_function_score = max(0.0, min(1.0, executive_function_score))
        
        # Determine executive function style based on dominant features
        executive_styles = {
            'memory_focused': working_memory + problem_solving,
            'flexibility_focused': cognitive_flexibility + task_switching,
            'control_focused': inhibitory_control + attention_control,
            'planning_focused': planning + decision_making,
            'problem_solver': problem_solving + decision_making,
            'balanced_executive': (working_memory + cognitive_flexibility) / 2
        }
        primary_style = max(executive_styles.items(), key=lambda x: x[1])[0]
        
        return {
            'executive_function_score': executive_function_score,
            'primary_executive_style': primary_style,
            'working_memory': working_memory,
            'cognitive_flexibility': cognitive_flexibility,
            'inhibitory_control': inhibitory_control,
            'planning': planning,
            'problem_solving': problem_solving,
            'attention_control': attention_control,
            'decision_making': decision_making,
            'task_switching': task_switching,
            'cognitive_control': inhibitory_control + attention_control,
            'mental_flexibility': cognitive_flexibility + task_switching,
            'strategic_thinking': planning + problem_solving,
            'cognitive_management': working_memory + decision_making,
            'executive_control': inhibitory_control + planning,
            'executive_function_profile': self.classify_executive_function_level(executive_function_score)
        }

    def _calculate_working_memory(self, information_dimension: float, entropy: float,
                                pattern_symmetry: float, spectral_entropy: float) -> float:
        """Calculate working memory from fingerprint features (DMIT principle)"""
        # DMIT research shows: High information dimension + entropy = working memory
        
        # Information dimension contribution
        if 1.5 <= information_dimension <= 2.0:
            info_score = (information_dimension - 1.5) / 0.5
        else:
            info_score = 0.5
        
        # Entropy contribution (higher entropy = more complex working memory)
        if entropy > 0:
            entropy_score = min(1.0, entropy / 8.0)  # Normalize to 0-1
        else:
            entropy_score = 0.5
        
        # Pattern symmetry contribution
        symmetry_score = min(1.0, pattern_symmetry)
        
        # Spectral entropy contribution
        spectral_score = min(1.0, spectral_entropy)
        
        # Combine scores
        working_memory = (info_score * 0.35 + entropy_score * 0.25 + symmetry_score * 0.25 + spectral_score * 0.15)
        return min(1.0, working_memory)
    
    def _calculate_cognitive_flexibility(self, ridge_density: float, clustering_coefficient: float,
                                       modularity: float, ridge_thickness: float) -> float:
        """Calculate cognitive flexibility from fingerprint features (DMIT principle)"""
        # DMIT research shows: High ridge density + clustering coefficient = cognitive flexibility
        
        # Ridge density contribution
        density_score = min(1.0, ridge_density)
        
        # Clustering coefficient contribution
        clustering_score = min(1.0, clustering_coefficient)
        
        # Modularity contribution
        modularity_score = min(1.0, modularity)
        
        # Ridge thickness contribution
        thickness_score = min(1.0, ridge_thickness)
        
        # Combine scores
        cognitive_flexibility = (density_score * 0.3 + clustering_score * 0.25 + modularity_score * 0.25 + thickness_score * 0.2)
        return min(1.0, cognitive_flexibility)
    
    def _calculate_inhibitory_control(self, correlation_dimension: float, graph_density: float,
                                    betweenness_centrality: float, information_dimension: float) -> float:
        """Calculate inhibitory control from fingerprint features (DMIT principle)"""
        # DMIT research shows: High correlation dimension + graph density = inhibitory control
        
        # Correlation dimension contribution
        if 1.5 <= correlation_dimension <= 2.0:
            corr_score = (correlation_dimension - 1.5) / 0.5
        else:
            corr_score = 0.5
        
        # Graph density contribution
        density_score = min(1.0, graph_density)
        
        # Betweenness centrality contribution
        centrality_score = min(1.0, betweenness_centrality)
        
        # Information dimension contribution
        if 1.5 <= information_dimension <= 2.0:
            info_score = (information_dimension - 1.5) / 0.5
        else:
            info_score = 0.5
        
        # Combine scores
        inhibitory_control = (corr_score * 0.3 + density_score * 0.25 + centrality_score * 0.25 + info_score * 0.2)
        return min(1.0, inhibitory_control)
    
    def _calculate_planning(self, community_cohesion: float, spectral_radius: float,
                          topological_complexity: float, euler_characteristic: int) -> float:
        """Calculate planning from fingerprint features (DMIT principle)"""
        # DMIT research shows: High community cohesion + spectral radius = planning
        
        # Community cohesion contribution
        cohesion_score = min(1.0, community_cohesion)
        
        # Spectral radius contribution
        if spectral_radius > 0:
            spectral_score = min(1.0, spectral_radius / 10.0)
        else:
            spectral_score = 0.5
        
        # Topological complexity contribution
        complexity_score = min(1.0, topological_complexity)
        
        # Euler characteristic contribution (more negative = more complex = better planning)
        if euler_characteristic < 0:
            euler_score = min(1.0, abs(euler_characteristic) / 10.0)
        else:
            euler_score = 0.3
        
        # Combine scores
        planning = (cohesion_score * 0.3 + spectral_score * 0.25 + complexity_score * 0.25 + euler_score * 0.2)
        return min(1.0, planning)
    
    def _calculate_problem_solving(self, tfrc: int, box_counting_dimension: float,
                                 h1_num_features: int, betti_1: int) -> float:
        """Calculate problem solving from fingerprint features (DMIT principle)"""
        # DMIT research shows: High ridge count + fractal dimension = problem solving
        
        # Ridge count contribution
        if tfrc > 0:
            ridge_score = min(1.0, tfrc / 1500.0)
        else:
            ridge_score = min(1.0, tfrc / 1500.0) if tfrc > 0 else 0.0
        
        # Fractal dimension contribution
        if 1.5 <= box_counting_dimension <= 2.0:
            fractal_score = (box_counting_dimension - 1.5) / 0.5
        else:
            fractal_score = 0.5
        
        # H1 features contribution (loops/holes - complexity in problem solving)
        h1_score = min(1.0, h1_num_features / 10.0)
        
        # Betti-1 contribution (problem solving loops)
        betti_score = min(1.0, betti_1 / 10.0)
        
        # Combine scores
        problem_solving = (ridge_score * 0.35 + fractal_score * 0.25 + h1_score * 0.25 + betti_score * 0.15)
        return min(1.0, problem_solving)
    
    def _calculate_attention_control(self, pattern_regularity: float, lacunarity: float,
                                   ridge_continuity: float, std_intensity: float) -> float:
        """Calculate attention control from fingerprint features (DMIT principle)"""
        # DMIT research shows: High pattern regularity + low lacunarity = attention control
        
        # Pattern regularity contribution
        regularity_score = min(1.0, pattern_regularity)
        
        # Lacunarity contribution (lower lacunarity = more regular = better attention control)
        lacunarity_score = max(0.0, 1.0 - lacunarity)
        
        # Ridge continuity contribution
        continuity_score = min(1.0, ridge_continuity)
        
        # Standard deviation contribution (lower variation = more consistent = better attention control)
        if std_intensity > 0:
            std_score = max(0.0, 1.0 - (std_intensity / 100.0))
        else:
            std_score = 0.5
        
        # Combine scores
        attention_control = (regularity_score * 0.3 + lacunarity_score * 0.25 + continuity_score * 0.25 + std_score * 0.2)
        return min(1.0, attention_control)
    
    def _calculate_decision_making(self, ridge_uniformity: float, pattern_type: str,
                                 ridge_curvature: float, spectral_energy: float) -> float:
        """Calculate decision making from fingerprint features (DMIT principle)"""
        # DMIT research shows: High ridge uniformity + pattern type = decision making
        
        # Ridge uniformity contribution
        uniformity_score = min(1.0, ridge_uniformity)
        
        # Pattern type contribution
        pattern_weights = {
            'whorl': 0.8,      # Complex patterns indicate decision making
            'loop': 0.7,       # Good decision making
            'arch': 0.6,       # Moderate decision making
            'tented_arch': 0.65, # Slightly better than plain arch
            'composite': 0.75   # Complex decision making patterns
        }
        pattern_score = pattern_weights.get(pattern_type.lower(), 0.6)
        
        # Ridge curvature contribution
        curvature_score = min(1.0, ridge_curvature)
        
        # Spectral energy contribution
        energy_score = min(1.0, spectral_energy)
        
        # Combine scores
        decision_making = (uniformity_score * 0.3 + pattern_score * 0.25 + curvature_score * 0.25 + energy_score * 0.2)
        return min(1.0, decision_making)
    
    def _calculate_task_switching(self, spectral_centroid: float, spectral_rolloff: float,
                                graph_density: float, topological_complexity: float) -> float:
        """Calculate task switching from fingerprint features (DMIT principle)"""
        # DMIT research shows: High spectral features + graph complexity = task switching
        
        # Spectral centroid contribution
        centroid_score = min(1.0, spectral_centroid)
        
        # Spectral rolloff contribution
        rolloff_score = min(1.0, spectral_rolloff)
        
        # Graph density contribution
        density_score = min(1.0, graph_density)
        
        # Topological complexity contribution
        complexity_score = min(1.0, topological_complexity)
        
        # Combine scores
        task_switching = (centroid_score * 0.3 + rolloff_score * 0.25 + density_score * 0.25 + complexity_score * 0.2)
        return min(1.0, task_switching)

    @staticmethod
    def classify_executive_function_level(score: float) -> str:
        if score >= 0.85:
            return "Exceptional Executive Function"
        elif score >= 0.75:
            return "High Executive Function"
        elif score >= 0.65:
            return "Above Average Executive Function"
        elif score >= 0.55:
            return "Average Executive Function"
        else:
            return "Developing Executive Function" 