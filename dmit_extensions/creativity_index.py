from typing import Dict, Any
from .base import DMITExtensionBase

class CreativityIndexExtension(DMITExtensionBase):
    """
    Extension for analyzing Creativity Index abilities from fingerprint features.
    Uses comprehensive DMIT scientific correlations between fingerprint patterns and creativity processes.
    """
    def analyze(self, features: Dict[str, Any]) -> Dict[str, Any]:
        # Extract comprehensive fingerprint features for creativity index analysis
        # Ridge count and density features
        # tfrc already extracted above
        ridge_density = features.get('ridge_density', 0.0)
        tfrc = features.get('tfrc', 0)  # Total Fingerprint Ridge Count
        ridge_continuity = features.get('ridge_continuity', 0.0)
        ridge_uniformity = features.get('ridge_uniformity', 0.0)
        ridge_thickness = features.get('mean_ridge_thickness', 0.0)
        ridge_orientation = features.get('mean_ridge_orientation', 0.0)
        ridge_curvature = features.get('mean_ridge_curvature', 0.0)
        
        # Pattern analysis features
        pattern_type = features.get('pattern_type', 'loop')
        pattern_symmetry = features.get('pattern_symmetry', 0.0)
        pattern_regularity = features.get('pattern_regularity', 0.0)
        whorl_complexity = features.get('whorl_complexity', 0.0)
        
        # Fractal and complexity features
        box_counting_dimension = features.get('box_counting_dimension', 1.5)
        correlation_dimension = features.get('correlation_dimension', 1.5)
        information_dimension = features.get('information_dimension', 1.5)
        lacunarity = features.get('lacunarity', 0.0)
        fractal_complexity = features.get('fractal_complexity', 0.0)
        
        # Graph and network features
        graph_density = features.get('graph_density', 0.0)
        clustering_coefficient = features.get('clustering_coefficient', 0.0)
        spectral_radius = features.get('spectral_radius', 0.0)
        betweenness_centrality = features.get('betweenness_centrality', 0.0)
        modularity = features.get('modularity', 0.0)
        community_cohesion = features.get('community_cohesion', 0.0)
        
        # Topological features
        euler_characteristic = features.get('euler_characteristic', 0)
        topological_complexity = features.get('topological_complexity', 0.0)
        h1_num_features = features.get('h1_num_features', 0)  # Loop/hole count
        betti_0 = features.get('betti_0', 0)  # Connected components
        betti_1 = features.get('betti_1', 0)  # Loops/holes
        
        # Statistical features
        entropy = features.get('entropy', 0.0)
        std_intensity = features.get('std_intensity', 0.0)
        minutiae_density = features.get('minutiae_density', 0.0)
        minutiae_count = features.get('minutiae_count', 0)
        
        # Spectral features
        spectral_energy = features.get('spectral_energy', 0.0)
        spectral_entropy = features.get('spectral_entropy', 0.0)
        spectral_centroid = features.get('spectral_centroid', 0.0)
        spectral_bandwidth = features.get('spectral_bandwidth', 0.0)
        spectral_rolloff = features.get('spectral_rolloff', 0.0)
        
        # Micro-features
        pore_density = features.get('pore_density', 0.0)
        pore_count = features.get('pore_count', 0)
        
        # Calculate creativity index abilities using comprehensive DMIT scientific correlations
        
        # 1. Divergent Thinking Analysis (DMIT Principle: High information dimension + entropy = divergent thinking)
        divergent_thinking = self._calculate_divergent_thinking(information_dimension, entropy, 
                                                              pattern_symmetry, spectral_entropy)
        
        # 2. Creative Fluency Analysis (DMIT Principle: High correlation dimension + graph density = creative fluency)
        creative_fluency = self._calculate_creative_fluency(correlation_dimension, graph_density, 
                                                          betweenness_centrality, information_dimension)
        
        # 3. Creative Flexibility Analysis (DMIT Principle: High ridge count + fractal complexity = creative flexibility)
        creative_flexibility = self._calculate_creative_flexibility(tfrc, box_counting_dimension, 
                                                                  h1_num_features, betti_1)
        
        # 4. Creative Originality Analysis (DMIT Principle: High spectral radius + topological complexity = creative originality)
        creative_originality = self._calculate_creative_originality(spectral_radius, topological_complexity, 
                                                                  euler_characteristic, spectral_bandwidth)
        
        # 5. Creative Elaboration Analysis (DMIT Principle: High ridge density + clustering coefficient = creative elaboration)
        creative_elaboration = self._calculate_creative_elaboration(ridge_density, clustering_coefficient, 
                                                                  modularity, ridge_thickness)
        
        # 6. Creative Expression Analysis (DMIT Principle: High ridge uniformity + pattern type = creative expression)
        creative_expression = self._calculate_creative_expression(ridge_uniformity, pattern_type, 
                                                                ridge_curvature, community_cohesion)
        
        # 7. Creative Problem Solving Analysis (DMIT Principle: High pattern regularity + low lacunarity = creative problem solving)
        creative_problem_solving = self._calculate_creative_problem_solving(pattern_regularity, lacunarity, 
                                                                          ridge_continuity, std_intensity)
        
        # 8. Creative Intelligence Analysis (DMIT Principle: High spectral features + graph complexity = creative intelligence)
        creative_intelligence = self._calculate_creative_intelligence(spectral_centroid, spectral_rolloff, 
                                                                    graph_density, topological_complexity)
        
        # Overall creativity index score (comprehensive weighted combination)
        creativity_index_score = (
            divergent_thinking * 0.20 +           # Divergent thinking is fundamental
            creative_fluency * 0.18 +             # Creative fluency is crucial
            creative_flexibility * 0.15 +         # Creative flexibility is important
            creative_originality * 0.15 +         # Creative originality is essential
            creative_elaboration * 0.12 +         # Creative elaboration
            creative_expression * 0.10 +          # Creative expression
            creative_problem_solving * 0.07 +     # Creative problem solving
            creative_intelligence * 0.03          # Creative intelligence
        )
        
        # Normalize to 0-1 range
        creativity_index_score = max(0.0, min(1.0, creativity_index_score))
        
        # Determine creativity index style based on dominant features
        creativity_styles = {
            'divergent': divergent_thinking + creative_intelligence,
            'fluent': creative_fluency + creative_expression,
            'flexible': creative_flexibility + creative_elaboration,
            'original': creative_originality + creative_problem_solving,
            'expressive': creative_expression + creative_fluency,
            'intelligent': creative_intelligence + divergent_thinking
        }
        primary_style = max(creativity_styles.items(), key=lambda x: x[1])[0]

        return {
            'creativity_index_score': creativity_index_score,
            'primary_creativity_style': primary_style,
            'divergent_thinking': divergent_thinking,
            'creative_fluency': creative_fluency,
            'creative_flexibility': creative_flexibility,
            'creative_originality': creative_originality,
            'creative_elaboration': creative_elaboration,
            'creative_expression': creative_expression,
            'creative_problem_solving': creative_problem_solving,
            'creative_intelligence': creative_intelligence,
            'creative_innovation': divergent_thinking + creative_originality,
            'creative_flow': creative_fluency + creative_expression,
            'creative_adaptability': creative_flexibility + creative_elaboration,
            'creative_insight': creative_originality + creative_problem_solving,
            'creative_mastery': creative_intelligence + creative_expression,
            'creativity_profile': self.classify_creativity_level(creativity_index_score)
        }

    def _calculate_divergent_thinking(self, information_dimension: float, entropy: float,
                                    pattern_symmetry: float, spectral_entropy: float) -> float:
        """Calculate divergent thinking from fingerprint features (DMIT principle)"""
        # DMIT research shows: High information dimension + entropy = divergent thinking
        
        # Information dimension contribution
        if 1.5 <= information_dimension <= 2.0:
            info_score = (information_dimension - 1.5) / 0.5
        else:
            info_score = 0.5
        
        # Entropy contribution (higher entropy = more complex divergent thinking)
        if entropy > 0:
            entropy_score = min(1.0, entropy / 8.0)  # Normalize to 0-1
        else:
            entropy_score = 0.5
        
        # Pattern symmetry contribution
        symmetry_score = min(1.0, pattern_symmetry)
        
        # Spectral entropy contribution
        spectral_score = min(1.0, spectral_entropy)
        
        # Combine scores
        divergent_thinking = (info_score * 0.35 + entropy_score * 0.25 + symmetry_score * 0.25 + spectral_score * 0.15)
        return min(1.0, divergent_thinking)
    
    def _calculate_creative_fluency(self, correlation_dimension: float, graph_density: float,
                                  betweenness_centrality: float, information_dimension: float) -> float:
        """Calculate creative fluency from fingerprint features (DMIT principle)"""
        # DMIT research shows: High correlation dimension + graph density = creative fluency
        
        # Correlation dimension contribution
        if 1.5 <= correlation_dimension <= 2.0:
            corr_score = (correlation_dimension - 1.5) / 0.5
        else:
            corr_score = 0.5
        
        # Graph density contribution
        density_score = min(1.0, graph_density)
        
        # Betweenness centrality contribution
        centrality_score = min(1.0, betweenness_centrality)
        
        # Information dimension contribution
        if 1.5 <= information_dimension <= 2.0:
            info_score = (information_dimension - 1.5) / 0.5
        else:
            info_score = 0.5
        
        # Combine scores
        creative_fluency = (corr_score * 0.3 + density_score * 0.25 + centrality_score * 0.25 + info_score * 0.2)
        return min(1.0, creative_fluency)
    
    def _calculate_creative_flexibility(self, tfrc: int, box_counting_dimension: float,
                                      h1_num_features: int, betti_1: int) -> float:
        """Calculate creative flexibility from fingerprint features (DMIT principle)"""
        # DMIT research shows: High ridge count + fractal complexity = creative flexibility
        
        # Ridge count contribution
        if tfrc > 0:
            ridge_score = min(1.0, tfrc / 1500.0)
        else:
            ridge_score = min(1.0, tfrc / 1500.0) if tfrc > 0 else 0.0
        
        # Fractal dimension contribution
        if 1.5 <= box_counting_dimension <= 2.0:
            fractal_score = (box_counting_dimension - 1.5) / 0.5
        else:
            fractal_score = 0.5
        
        # H1 features contribution (loops/holes - complexity in creative flexibility)
        h1_score = min(1.0, h1_num_features / 10.0)
        
        # Betti-1 contribution (creative flexibility loops)
        betti_score = min(1.0, betti_1 / 10.0)
        
        # Combine scores
        creative_flexibility = (ridge_score * 0.35 + fractal_score * 0.25 + h1_score * 0.25 + betti_score * 0.15)
        return min(1.0, creative_flexibility)
    
    def _calculate_creative_originality(self, spectral_radius: float, topological_complexity: float,
                                      euler_characteristic: int, spectral_bandwidth: float) -> float:
        """Calculate creative originality from fingerprint features (DMIT principle)"""
        # DMIT research shows: High spectral radius + topological complexity = creative originality
        
        # Spectral radius contribution
        if spectral_radius > 0:
            spectral_score = min(1.0, spectral_radius / 10.0)
        else:
            spectral_score = 0.5
        
        # Topological complexity contribution
        complexity_score = min(1.0, topological_complexity)
        
        # Euler characteristic contribution
        if euler_characteristic > 0:
            euler_score = min(1.0, euler_characteristic / 10.0)
        else:
            euler_score = 0.5
        
        # Spectral bandwidth contribution
        bandwidth_score = min(1.0, spectral_bandwidth)
        
        # Combine scores
        creative_originality = (spectral_score * 0.3 + complexity_score * 0.25 + euler_score * 0.25 + bandwidth_score * 0.2)
        return min(1.0, creative_originality)
    
    def _calculate_creative_elaboration(self, ridge_density: float, clustering_coefficient: float,
                                      modularity: float, ridge_thickness: float) -> float:
        """Calculate creative elaboration from fingerprint features (DMIT principle)"""
        # DMIT research shows: High ridge density + clustering coefficient = creative elaboration
        
        # Ridge density contribution
        density_score = min(1.0, ridge_density)
        
        # Clustering coefficient contribution
        clustering_score = min(1.0, clustering_coefficient)
        
        # Modularity contribution
        modularity_score = min(1.0, modularity)
        
        # Ridge thickness contribution
        thickness_score = min(1.0, ridge_thickness)
        
        # Combine scores
        creative_elaboration = (density_score * 0.3 + clustering_score * 0.25 + modularity_score * 0.25 + thickness_score * 0.2)
        return min(1.0, creative_elaboration)
    
    def _calculate_creative_expression(self, ridge_uniformity: float, pattern_type: str,
                                     ridge_curvature: float, community_cohesion: float) -> float:
        """Calculate creative expression from fingerprint features (DMIT principle)"""
        # DMIT research shows: High ridge uniformity + pattern type = creative expression
        
        # Ridge uniformity contribution
        uniformity_score = min(1.0, ridge_uniformity)
        
        # Pattern type contribution
        pattern_weights = {
            'whorl': 0.8,      # Complex patterns indicate good creative expression
            'loop': 0.7,       # Good creative expression
            'arch': 0.6,       # Moderate creative expression
            'tented_arch': 0.65, # Slightly better than plain arch
            'composite': 0.75   # Complex creative expression patterns
        }
        pattern_score = pattern_weights.get(pattern_type.lower(), 0.6)
        
        # Ridge curvature contribution
        curvature_score = min(1.0, ridge_curvature)
        
        # Community cohesion contribution
        cohesion_score = min(1.0, community_cohesion)
        
        # Combine scores
        creative_expression = (uniformity_score * 0.3 + pattern_score * 0.25 + curvature_score * 0.25 + cohesion_score * 0.2)
        return min(1.0, creative_expression)
    
    def _calculate_creative_problem_solving(self, pattern_regularity: float, lacunarity: float,
                                          ridge_continuity: float, std_intensity: float) -> float:
        """Calculate creative problem solving from fingerprint features (DMIT principle)"""
        # DMIT research shows: High pattern regularity + low lacunarity = creative problem solving
        
        # Pattern regularity contribution
        regularity_score = min(1.0, pattern_regularity)
        
        # Lacunarity contribution (lower lacunarity = more regular = better creative problem solving)
        lacunarity_score = max(0.0, 1.0 - lacunarity)
        
        # Ridge continuity contribution
        continuity_score = min(1.0, ridge_continuity)
        
        # Standard deviation contribution (lower variation = more consistent = better creative problem solving)
        if std_intensity > 0:
            std_score = max(0.0, 1.0 - (std_intensity / 100.0))
        else:
            std_score = 0.5
        
        # Combine scores
        creative_problem_solving = (regularity_score * 0.3 + lacunarity_score * 0.25 + continuity_score * 0.25 + std_score * 0.2)
        return min(1.0, creative_problem_solving)
    
    def _calculate_creative_intelligence(self, spectral_centroid: float, spectral_rolloff: float,
                                       graph_density: float, topological_complexity: float) -> float:
        """Calculate creative intelligence from fingerprint features (DMIT principle)"""
        # DMIT research shows: High spectral features + graph complexity = creative intelligence
        
        # Spectral centroid contribution
        centroid_score = min(1.0, spectral_centroid)
        
        # Spectral rolloff contribution
        rolloff_score = min(1.0, spectral_rolloff)
        
        # Graph density contribution
        density_score = min(1.0, graph_density)
        
        # Topological complexity contribution
        complexity_score = min(1.0, topological_complexity)
        
        # Combine scores
        creative_intelligence = (centroid_score * 0.3 + rolloff_score * 0.25 + density_score * 0.25 + complexity_score * 0.2)
        return min(1.0, creative_intelligence)

    @staticmethod
    def classify_creativity_level(score: float) -> str:
        """Classify creativity index level based on score"""
        if score >= 0.8:
            return "Excellent"
        elif score >= 0.7:
            return "Very Good"
        elif score >= 0.6:
            return "Good"
        elif score >= 0.5:
            return "Average"
        elif score >= 0.4:
            return "Below Average"
        else:
            return "Needs Development" 