from typing import Dict, Any
from .base import DMITExtensionBase

class InnovationCreativityExtension(DMITExtensionBase):
    """
    Extension for analyzing Innovation and Creativity abilities from fingerprint features.
    Uses comprehensive DMIT scientific correlations between fingerprint patterns and creative capabilities.
    """
    def analyze(self, features: Dict[str, Any]) -> Dict[str, Any]:
        # Extract comprehensive fingerprint features for innovation creativity analysis
        # Ridge count and density features
        # tfrc already extracted above
        ridge_density = features.get('ridge_density', 0.0)
        tfrc = features.get('tfrc', 0)  # Total Fingerprint Ridge Count
        ridge_continuity = features.get('ridge_continuity', 0.0)
        ridge_uniformity = features.get('ridge_uniformity', 0.0)
        ridge_thickness = features.get('mean_ridge_thickness', 0.0)
        ridge_orientation = features.get('mean_ridge_orientation', 0.0)
        ridge_curvature = features.get('mean_ridge_curvature', 0.0)
        
        # Pattern analysis features
        pattern_type = features.get('pattern_type', 'loop')
        pattern_symmetry = features.get('pattern_symmetry', 0.0)
        pattern_regularity = features.get('pattern_regularity', 0.0)
        whorl_complexity = features.get('whorl_complexity', 0.0)
        
        # Fractal and complexity features
        box_counting_dimension = features.get('box_counting_dimension', 1.5)
        correlation_dimension = features.get('correlation_dimension', 1.5)
        information_dimension = features.get('information_dimension', 1.5)
        lacunarity = features.get('lacunarity', 0.0)
        fractal_complexity = features.get('fractal_complexity', 0.0)
        
        # Graph and network features
        graph_density = features.get('graph_density', 0.0)
        clustering_coefficient = features.get('clustering_coefficient', 0.0)
        spectral_radius = features.get('spectral_radius', 0.0)
        betweenness_centrality = features.get('betweenness_centrality', 0.0)
        modularity = features.get('modularity', 0.0)
        community_cohesion = features.get('community_cohesion', 0.0)
        
        # Topological features
        euler_characteristic = features.get('euler_characteristic', 0)
        topological_complexity = features.get('topological_complexity', 0.0)
        h1_num_features = features.get('h1_num_features', 0)  # Loop/hole count
        betti_0 = features.get('betti_0', 0)  # Connected components
        betti_1 = features.get('betti_1', 0)  # Loops/holes
        
        # Statistical features
        entropy = features.get('entropy', 0.0)
        std_intensity = features.get('std_intensity', 0.0)
        minutiae_density = features.get('minutiae_density', 0.0)
        minutiae_count = features.get('minutiae_count', 0)
        
        # Spectral features
        spectral_energy = features.get('spectral_energy', 0.0)
        spectral_entropy = features.get('spectral_entropy', 0.0)
        spectral_centroid = features.get('spectral_centroid', 0.0)
        spectral_bandwidth = features.get('spectral_bandwidth', 0.0)
        spectral_rolloff = features.get('spectral_rolloff', 0.0)
        
        # Micro-features
        pore_density = features.get('pore_density', 0.0)
        pore_count = features.get('pore_count', 0)
        
        # Calculate innovation creativity abilities using comprehensive DMIT scientific correlations
        
        # 1. Creative Thinking Analysis (DMIT Principle: High information dimension + entropy = creative thinking)
        creative_thinking = self._calculate_creative_thinking(information_dimension, entropy, 
                                                            pattern_symmetry, spectral_entropy)
        
        # 2. Innovation Generation Analysis (DMIT Principle: High ridge density + clustering coefficient = innovation generation)
        innovation_generation = self._calculate_innovation_generation(ridge_density, clustering_coefficient, 
                                                                    modularity, ridge_thickness)
        
        # 3. Problem Solving Analysis (DMIT Principle: High correlation dimension + graph density = problem solving)
        problem_solving = self._calculate_problem_solving(correlation_dimension, graph_density, 
                                                        betweenness_centrality, information_dimension)
        
        # 4. Idea Synthesis Analysis (DMIT Principle: High community cohesion + spectral radius = idea synthesis)
        idea_synthesis = self._calculate_idea_synthesis(community_cohesion, spectral_radius, 
                                                      topological_complexity, euler_characteristic)
        
        # 5. Divergent Thinking Analysis (DMIT Principle: High ridge count + fractal dimension = divergent thinking)
        divergent_thinking = self._calculate_divergent_thinking(tfrc, box_counting_dimension, 
                                                              h1_num_features, betti_1)
        
        # 6. Pattern Recognition Analysis (DMIT Principle: High pattern regularity + low lacunarity = pattern recognition)
        pattern_recognition = self._calculate_pattern_recognition(pattern_regularity, lacunarity, 
                                                                ridge_continuity, std_intensity)
        
        # 7. Conceptual Integration Analysis (DMIT Principle: High ridge uniformity + pattern type = conceptual integration)
        conceptual_integration = self._calculate_conceptual_integration(ridge_uniformity, pattern_type, 
                                                                      ridge_curvature, spectral_energy)
        
        # 8. Creative Adaptability Analysis (DMIT Principle: High spectral features + graph complexity = creative adaptability)
        creative_adaptability = self._calculate_creative_adaptability(spectral_centroid, spectral_rolloff, 
                                                                    graph_density, topological_complexity)
        
        # Calculate overall innovation creativity score
        innovation_creativity_score = (
            creative_thinking * 0.20 +               # Creative thinking is fundamental
            innovation_generation * 0.18 +           # Innovation generation is crucial
            problem_solving * 0.15 +                 # Problem solving is important
            idea_synthesis * 0.15 +                  # Idea synthesis is essential
            divergent_thinking * 0.12 +              # Divergent thinking
            pattern_recognition * 0.10 +             # Pattern recognition
            conceptual_integration * 0.07 +          # Conceptual integration
            creative_adaptability * 0.03             # Creative adaptability
        )
        
        # Normalize to 0-1 range
        innovation_creativity_score = max(0.0, min(1.0, innovation_creativity_score))
        
        # Determine innovation creativity style based on dominant features
        innovation_styles = {
            'creative_thinker': creative_thinking + divergent_thinking,
            'innovation_generator': innovation_generation + idea_synthesis,
            'problem_solver': problem_solving + pattern_recognition,
            'conceptual_integrator': conceptual_integration + creative_adaptability,
            'idea_synthesizer': idea_synthesis + creative_thinking,
            'balanced_innovator': (creative_thinking + innovation_generation) / 2
        }
        primary_style = max(innovation_styles.items(), key=lambda x: x[1])[0]
        
        return {
            'innovation_creativity_score': innovation_creativity_score,
            'primary_innovation_style': primary_style,
            'creative_thinking': creative_thinking,
            'innovation_generation': innovation_generation,
            'problem_solving': problem_solving,
            'idea_synthesis': idea_synthesis,
            'divergent_thinking': divergent_thinking,
            'pattern_recognition': pattern_recognition,
            'conceptual_integration': conceptual_integration,
            'creative_adaptability': creative_adaptability,
            'creative_potential': creative_thinking + innovation_generation,
            'innovation_capacity': innovation_generation + idea_synthesis,
            'problem_innovation': problem_solving + divergent_thinking,
            'creative_synthesis': idea_synthesis + conceptual_integration,
            'adaptive_creativity': creative_adaptability + pattern_recognition,
            'innovation_creativity_profile': self.classify_innovation_creativity_level(innovation_creativity_score)
        }

    def _calculate_creative_thinking(self, information_dimension: float, entropy: float,
                                   pattern_symmetry: float, spectral_entropy: float) -> float:
        """Calculate creative thinking from fingerprint features (DMIT principle)"""
        # DMIT research shows: High information dimension + entropy = creative thinking
        
        # Information dimension contribution
        if 1.5 <= information_dimension <= 2.0:
            info_score = (information_dimension - 1.5) / 0.5
        else:
            info_score = 0.5
        
        # Entropy contribution (higher entropy = more complex creative thinking)
        if entropy > 0:
            entropy_score = min(1.0, entropy / 8.0)  # Normalize to 0-1
        else:
            entropy_score = 0.5
        
        # Pattern symmetry contribution
        symmetry_score = min(1.0, pattern_symmetry)
        
        # Spectral entropy contribution
        spectral_score = min(1.0, spectral_entropy)
        
        # Combine scores
        creative_thinking = (info_score * 0.35 + entropy_score * 0.25 + symmetry_score * 0.25 + spectral_score * 0.15)
        return min(1.0, creative_thinking)
    
    def _calculate_innovation_generation(self, ridge_density: float, clustering_coefficient: float,
                                       modularity: float, ridge_thickness: float) -> float:
        """Calculate innovation generation from fingerprint features (DMIT principle)"""
        # DMIT research shows: High ridge density + clustering coefficient = innovation generation
        
        # Ridge density contribution
        density_score = min(1.0, ridge_density)
        
        # Clustering coefficient contribution
        clustering_score = min(1.0, clustering_coefficient)
        
        # Modularity contribution
        modularity_score = min(1.0, modularity)
        
        # Ridge thickness contribution
        thickness_score = min(1.0, ridge_thickness)
        
        # Combine scores
        innovation_generation = (density_score * 0.3 + clustering_score * 0.25 + modularity_score * 0.25 + thickness_score * 0.2)
        return min(1.0, innovation_generation)
    
    def _calculate_problem_solving(self, correlation_dimension: float, graph_density: float,
                                 betweenness_centrality: float, information_dimension: float) -> float:
        """Calculate problem solving from fingerprint features (DMIT principle)"""
        # DMIT research shows: High correlation dimension + graph density = problem solving
        
        # Correlation dimension contribution
        if 1.5 <= correlation_dimension <= 2.0:
            corr_score = (correlation_dimension - 1.5) / 0.5
        else:
            corr_score = 0.5
        
        # Graph density contribution
        density_score = min(1.0, graph_density)
        
        # Betweenness centrality contribution
        centrality_score = min(1.0, betweenness_centrality)
        
        # Information dimension contribution
        if 1.5 <= information_dimension <= 2.0:
            info_score = (information_dimension - 1.5) / 0.5
        else:
            info_score = 0.5
        
        # Combine scores
        problem_solving = (corr_score * 0.3 + density_score * 0.25 + centrality_score * 0.25 + info_score * 0.2)
        return min(1.0, problem_solving)
    
    def _calculate_idea_synthesis(self, community_cohesion: float, spectral_radius: float,
                                topological_complexity: float, euler_characteristic: int) -> float:
        """Calculate idea synthesis from fingerprint features (DMIT principle)"""
        # DMIT research shows: High community cohesion + spectral radius = idea synthesis
        
        # Community cohesion contribution
        cohesion_score = min(1.0, community_cohesion)
        
        # Spectral radius contribution
        if spectral_radius > 0:
            spectral_score = min(1.0, spectral_radius / 10.0)
        else:
            spectral_score = 0.5
        
        # Topological complexity contribution
        complexity_score = min(1.0, topological_complexity)
        
        # Euler characteristic contribution (more negative = more complex = better idea synthesis)
        if euler_characteristic < 0:
            euler_score = min(1.0, abs(euler_characteristic) / 10.0)
        else:
            euler_score = 0.3
        
        # Combine scores
        idea_synthesis = (cohesion_score * 0.3 + spectral_score * 0.25 + complexity_score * 0.25 + euler_score * 0.2)
        return min(1.0, idea_synthesis)
    
    def _calculate_divergent_thinking(self, tfrc: int, box_counting_dimension: float,
                                    h1_num_features: int, betti_1: int) -> float:
        """Calculate divergent thinking from fingerprint features (DMIT principle)"""
        # DMIT research shows: High ridge count + fractal dimension = divergent thinking
        
        # Ridge count contribution
        if tfrc > 0:
            ridge_score = min(1.0, tfrc / 1500.0)
        else:
            ridge_score = min(1.0, tfrc / 1500.0) if tfrc > 0 else 0.0
        
        # Fractal dimension contribution
        if 1.5 <= box_counting_dimension <= 2.0:
            fractal_score = (box_counting_dimension - 1.5) / 0.5
        else:
            fractal_score = 0.5
        
        # H1 features contribution (loops/holes - complexity in divergent thinking)
        h1_score = min(1.0, h1_num_features / 10.0)
        
        # Betti-1 contribution (divergent thinking loops)
        betti_score = min(1.0, betti_1 / 10.0)
        
        # Combine scores
        divergent_thinking = (ridge_score * 0.35 + fractal_score * 0.25 + h1_score * 0.25 + betti_score * 0.15)
        return min(1.0, divergent_thinking)
    
    def _calculate_pattern_recognition(self, pattern_regularity: float, lacunarity: float,
                                     ridge_continuity: float, std_intensity: float) -> float:
        """Calculate pattern recognition from fingerprint features (DMIT principle)"""
        # DMIT research shows: High pattern regularity + low lacunarity = pattern recognition
        
        # Pattern regularity contribution
        regularity_score = min(1.0, pattern_regularity)
        
        # Lacunarity contribution (lower lacunarity = more regular = better pattern recognition)
        lacunarity_score = max(0.0, 1.0 - lacunarity)
        
        # Ridge continuity contribution
        continuity_score = min(1.0, ridge_continuity)
        
        # Standard deviation contribution (lower variation = more consistent = better pattern recognition)
        if std_intensity > 0:
            std_score = max(0.0, 1.0 - (std_intensity / 100.0))
        else:
            std_score = 0.5
        
        # Combine scores
        pattern_recognition = (regularity_score * 0.3 + lacunarity_score * 0.25 + continuity_score * 0.25 + std_score * 0.2)
        return min(1.0, pattern_recognition)
    
    def _calculate_conceptual_integration(self, ridge_uniformity: float, pattern_type: str,
                                        ridge_curvature: float, spectral_energy: float) -> float:
        """Calculate conceptual integration from fingerprint features (DMIT principle)"""
        # DMIT research shows: High ridge uniformity + pattern type = conceptual integration
        
        # Ridge uniformity contribution
        uniformity_score = min(1.0, ridge_uniformity)
        
        # Pattern type contribution
        pattern_weights = {
            'whorl': 0.8,      # Complex patterns indicate conceptual integration
            'loop': 0.7,       # Good conceptual integration
            'arch': 0.6,       # Moderate conceptual integration
            'tented_arch': 0.65, # Slightly better than plain arch
            'composite': 0.75   # Complex conceptual integration patterns
        }
        pattern_score = pattern_weights.get(pattern_type.lower(), 0.6)
        
        # Ridge curvature contribution
        curvature_score = min(1.0, ridge_curvature)
        
        # Spectral energy contribution
        energy_score = min(1.0, spectral_energy)
        
        # Combine scores
        conceptual_integration = (uniformity_score * 0.3 + pattern_score * 0.25 + curvature_score * 0.25 + energy_score * 0.2)
        return min(1.0, conceptual_integration)
    
    def _calculate_creative_adaptability(self, spectral_centroid: float, spectral_rolloff: float,
                                       graph_density: float, topological_complexity: float) -> float:
        """Calculate creative adaptability from fingerprint features (DMIT principle)"""
        # DMIT research shows: High spectral features + graph complexity = creative adaptability
        
        # Spectral centroid contribution
        centroid_score = min(1.0, spectral_centroid)
        
        # Spectral rolloff contribution
        rolloff_score = min(1.0, spectral_rolloff)
        
        # Graph density contribution
        density_score = min(1.0, graph_density)
        
        # Topological complexity contribution
        complexity_score = min(1.0, topological_complexity)
        
        # Combine scores
        creative_adaptability = (centroid_score * 0.3 + rolloff_score * 0.25 + density_score * 0.25 + complexity_score * 0.2)
        return min(1.0, creative_adaptability)

    @staticmethod
    def classify_innovation_creativity_level(score: float) -> str:
        if score >= 0.85:
            return "Exceptional Innovation and Creativity"
        elif score >= 0.75:
            return "High Innovation and Creativity"
        elif score >= 0.65:
            return "Above Average Innovation and Creativity"
        elif score >= 0.55:
            return "Average Innovation and Creativity"
        else:
            return "Developing Innovation and Creativity" 
