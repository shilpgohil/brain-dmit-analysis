from typing import Dict, Any
from .base import DMITExtensionBase

class SpatialIntelligenceExtension(DMITExtensionBase):
    """
    Extension for analyzing Spatial Intelligence and visual thinking abilities from fingerprint features.
    Uses comprehensive DMIT scientific correlations between fingerprint patterns and spatial capabilities.
    """
    def analyze(self, features: Dict[str, Any]) -> Dict[str, Any]:
        # Extract comprehensive fingerprint features for spatial intelligence analysis
        # Ridge count and density features
        # tfrc already extracted above
        ridge_density = features.get('ridge_density', 0.0)
        tfrc = features.get('tfrc', 0)  # Total Fingerprint Ridge Count
        ridge_continuity = features.get('ridge_continuity', 0.0)
        ridge_uniformity = features.get('ridge_uniformity', 0.0)
        ridge_thickness = features.get('mean_ridge_thickness', 0.0)
        ridge_orientation = features.get('mean_ridge_orientation', 0.0)
        ridge_curvature = features.get('mean_ridge_curvature', 0.0)
        
        # Pattern analysis features
        pattern_type = features.get('pattern_type', 'loop')
        pattern_symmetry = features.get('pattern_symmetry', 0.0)
        pattern_regularity = features.get('pattern_regularity', 0.0)
        whorl_complexity = features.get('whorl_complexity', 0.0)
        
        # Fractal and complexity features
        box_counting_dimension = features.get('box_counting_dimension', 1.5)
        correlation_dimension = features.get('correlation_dimension', 1.5)
        information_dimension = features.get('information_dimension', 1.5)
        lacunarity = features.get('lacunarity', 0.0)
        fractal_complexity = features.get('fractal_complexity', 0.0)
        
        # Graph and network features
        graph_density = features.get('graph_density', 0.0)
        clustering_coefficient = features.get('clustering_coefficient', 0.0)
        spectral_radius = features.get('spectral_radius', 0.0)
        betweenness_centrality = features.get('betweenness_centrality', 0.0)
        modularity = features.get('modularity', 0.0)
        community_cohesion = features.get('community_cohesion', 0.0)
        
        # Topological features
        euler_characteristic = features.get('euler_characteristic', 0)
        topological_complexity = features.get('topological_complexity', 0.0)
        h1_num_features = features.get('h1_num_features', 0)  # Loop/hole count
        betti_0 = features.get('betti_0', 0)  # Connected components
        betti_1 = features.get('betti_1', 0)  # Loops/holes
        
        # Statistical features
        entropy = features.get('entropy', 0.0)
        std_intensity = features.get('std_intensity', 0.0)
        minutiae_density = features.get('minutiae_density', 0.0)
        minutiae_count = features.get('minutiae_count', 0)
        
        # Spectral features
        spectral_energy = features.get('spectral_energy', 0.0)
        spectral_entropy = features.get('spectral_entropy', 0.0)
        spectral_centroid = features.get('spectral_centroid', 0.0)
        spectral_bandwidth = features.get('spectral_bandwidth', 0.0)
        spectral_rolloff = features.get('spectral_rolloff', 0.0)
        
        # Micro-features
        pore_density = features.get('pore_density', 0.0)
        pore_count = features.get('pore_count', 0)
        
        # Calculate spatial intelligence abilities using comprehensive DMIT scientific correlations
        
        # 1. Visual Perception Analysis (DMIT Principle: High information dimension + entropy = visual perception)
        visual_perception = self._calculate_visual_perception(information_dimension, entropy, 
                                                            pattern_symmetry, spectral_entropy)
        
        # 2. Spatial Orientation Analysis (DMIT Principle: High ridge density + clustering coefficient = spatial orientation)
        spatial_orientation = self._calculate_spatial_orientation(ridge_density, clustering_coefficient, 
                                                                modularity, ridge_thickness)
        
        # 3. Mental Rotation Analysis (DMIT Principle: High correlation dimension + graph density = mental rotation)
        mental_rotation = self._calculate_mental_rotation(correlation_dimension, graph_density, 
                                                        betweenness_centrality, information_dimension)
        
        # 4. Pattern Visualization Analysis (DMIT Principle: High community cohesion + spectral radius = pattern visualization)
        pattern_visualization = self._calculate_pattern_visualization(community_cohesion, spectral_radius, 
                                                                    topological_complexity, euler_characteristic)
        
        # 5. Spatial Memory Analysis (DMIT Principle: High ridge count + fractal dimension = spatial memory)
        spatial_memory = self._calculate_spatial_memory(tfrc, box_counting_dimension, 
                                                      h1_num_features, betti_1)
        
        # 6. Visual Organization Analysis (DMIT Principle: High pattern regularity + low lacunarity = visual organization)
        visual_organization = self._calculate_visual_organization(pattern_regularity, lacunarity, 
                                                                ridge_continuity, std_intensity)
        
        # 7. Spatial Reasoning Analysis (DMIT Principle: High ridge uniformity + pattern type = spatial reasoning)
        spatial_reasoning = self._calculate_spatial_reasoning(ridge_uniformity, pattern_type, 
                                                            ridge_curvature, spectral_energy)
        
        # 8. Visual Adaptability Analysis (DMIT Principle: High spectral features + graph complexity = visual adaptability)
        visual_adaptability = self._calculate_visual_adaptability(spectral_centroid, spectral_rolloff, 
                                                                graph_density, topological_complexity)
        
        # Calculate overall spatial intelligence score
        spatial_intelligence_score = (
            visual_perception * 0.20 +               # Visual perception is fundamental
            spatial_orientation * 0.18 +             # Spatial orientation is crucial
            mental_rotation * 0.15 +                 # Mental rotation is important
            pattern_visualization * 0.15 +           # Pattern visualization is essential
            spatial_memory * 0.12 +                  # Spatial memory
            visual_organization * 0.10 +             # Visual organization
            spatial_reasoning * 0.07 +               # Spatial reasoning
            visual_adaptability * 0.03               # Visual adaptability
        )
        
        # Normalize to 0-1 range
        spatial_intelligence_score = max(0.0, min(1.0, spatial_intelligence_score))
        
        # Determine spatial intelligence style based on dominant features
        spatial_styles = {
            'visual_perceiver': visual_perception + visual_organization,
            'spatial_navigator': spatial_orientation + spatial_memory,
            'mental_rotator': mental_rotation + spatial_reasoning,
            'pattern_visualizer': pattern_visualization + visual_adaptability,
            'spatial_thinker': spatial_reasoning + mental_rotation,
            'balanced_spatial': (visual_perception + spatial_orientation) / 2
        }
        primary_style = max(spatial_styles.items(), key=lambda x: x[1])[0]
        
        return {
            'spatial_intelligence_score': spatial_intelligence_score,
            'primary_spatial_style': primary_style,
            'visual_perception': visual_perception,
            'spatial_orientation': spatial_orientation,
            'mental_rotation': mental_rotation,
            'pattern_visualization': pattern_visualization,
            'spatial_memory': spatial_memory,
            'visual_organization': visual_organization,
            'spatial_reasoning': spatial_reasoning,
            'visual_adaptability': visual_adaptability,
            'visual_capacity': visual_perception + visual_organization,
            'spatial_capacity': spatial_orientation + spatial_memory,
            'mental_visualization': mental_rotation + pattern_visualization,
            'spatial_adaptability': visual_adaptability + spatial_reasoning,
            'spatial_perception': visual_perception + spatial_orientation,
            'spatial_intelligence_profile': self.classify_spatial_intelligence_level(spatial_intelligence_score)
        }

    def _calculate_visual_perception(self, information_dimension: float, entropy: float,
                                   pattern_symmetry: float, spectral_entropy: float) -> float:
        """Calculate visual perception from fingerprint features (DMIT principle)"""
        # DMIT research shows: High information dimension + entropy = visual perception
        
        # Information dimension contribution
        if 1.5 <= information_dimension <= 2.0:
            info_score = (information_dimension - 1.5) / 0.5
        else:
            info_score = 0.5
        
        # Entropy contribution (higher entropy = more complex visual perception)
        if entropy > 0:
            entropy_score = min(1.0, entropy / 8.0)  # Normalize to 0-1
        else:
            entropy_score = 0.5
        
        # Pattern symmetry contribution
        symmetry_score = min(1.0, pattern_symmetry)
        
        # Spectral entropy contribution
        spectral_score = min(1.0, spectral_entropy)
        
        # Combine scores
        visual_perception = (info_score * 0.35 + entropy_score * 0.25 + symmetry_score * 0.25 + spectral_score * 0.15)
        return min(1.0, visual_perception)
    
    def _calculate_spatial_orientation(self, ridge_density: float, clustering_coefficient: float,
                                     modularity: float, ridge_thickness: float) -> float:
        """Calculate spatial orientation from fingerprint features (DMIT principle)"""
        # DMIT research shows: High ridge density + clustering coefficient = spatial orientation
        
        # Ridge density contribution
        density_score = min(1.0, ridge_density)
        
        # Clustering coefficient contribution
        clustering_score = min(1.0, clustering_coefficient)
        
        # Modularity contribution
        modularity_score = min(1.0, modularity)
        
        # Ridge thickness contribution
        thickness_score = min(1.0, ridge_thickness)
        
        # Combine scores
        spatial_orientation = (density_score * 0.3 + clustering_score * 0.25 + modularity_score * 0.25 + thickness_score * 0.2)
        return min(1.0, spatial_orientation)
    
    def _calculate_mental_rotation(self, correlation_dimension: float, graph_density: float,
                                 betweenness_centrality: float, information_dimension: float) -> float:
        """Calculate mental rotation from fingerprint features (DMIT principle)"""
        # DMIT research shows: High correlation dimension + graph density = mental rotation
        
        # Correlation dimension contribution
        if 1.5 <= correlation_dimension <= 2.0:
            corr_score = (correlation_dimension - 1.5) / 0.5
        else:
            corr_score = 0.5
        
        # Graph density contribution
        density_score = min(1.0, graph_density)
        
        # Betweenness centrality contribution
        centrality_score = min(1.0, betweenness_centrality)
        
        # Information dimension contribution
        if 1.5 <= information_dimension <= 2.0:
            info_score = (information_dimension - 1.5) / 0.5
        else:
            info_score = 0.5
        
        # Combine scores
        mental_rotation = (corr_score * 0.3 + density_score * 0.25 + centrality_score * 0.25 + info_score * 0.2)
        return min(1.0, mental_rotation)
    
    def _calculate_pattern_visualization(self, community_cohesion: float, spectral_radius: float,
                                       topological_complexity: float, euler_characteristic: int) -> float:
        """Calculate pattern visualization from fingerprint features (DMIT principle)"""
        # DMIT research shows: High community cohesion + spectral radius = pattern visualization
        
        # Community cohesion contribution
        cohesion_score = min(1.0, community_cohesion)
        
        # Spectral radius contribution
        if spectral_radius > 0:
            spectral_score = min(1.0, spectral_radius / 10.0)
        else:
            spectral_score = 0.5
        
        # Topological complexity contribution
        complexity_score = min(1.0, topological_complexity)
        
        # Euler characteristic contribution (more negative = more complex = better pattern visualization)
        if euler_characteristic < 0:
            euler_score = min(1.0, abs(euler_characteristic) / 10.0)
        else:
            euler_score = 0.3
        
        # Combine scores
        pattern_visualization = (cohesion_score * 0.3 + spectral_score * 0.25 + complexity_score * 0.25 + euler_score * 0.2)
        return min(1.0, pattern_visualization)
    
    def _calculate_spatial_memory(self, tfrc: int, box_counting_dimension: float,
                                h1_num_features: int, betti_1: int) -> float:
        """Calculate spatial memory from fingerprint features (DMIT principle)"""
        # DMIT research shows: High ridge count + fractal dimension = spatial memory
        
        # Ridge count contribution
        if tfrc > 0:
            ridge_score = min(1.0, tfrc / 1500.0)
        else:
            ridge_score = min(1.0, tfrc / 1500.0) if tfrc > 0 else 0.0
        
        # Fractal dimension contribution
        if 1.5 <= box_counting_dimension <= 2.0:
            fractal_score = (box_counting_dimension - 1.5) / 0.5
        else:
            fractal_score = 0.5
        
        # H1 features contribution (loops/holes - complexity in spatial memory)
        h1_score = min(1.0, h1_num_features / 10.0)
        
        # Betti-1 contribution (spatial memory loops)
        betti_score = min(1.0, betti_1 / 10.0)
        
        # Combine scores
        spatial_memory = (ridge_score * 0.35 + fractal_score * 0.25 + h1_score * 0.25 + betti_score * 0.15)
        return min(1.0, spatial_memory)
    
    def _calculate_visual_organization(self, pattern_regularity: float, lacunarity: float,
                                     ridge_continuity: float, std_intensity: float) -> float:
        """Calculate visual organization from fingerprint features (DMIT principle)"""
        # DMIT research shows: High pattern regularity + low lacunarity = visual organization
        
        # Pattern regularity contribution
        regularity_score = min(1.0, pattern_regularity)
        
        # Lacunarity contribution (lower lacunarity = more regular = better visual organization)
        lacunarity_score = max(0.0, 1.0 - lacunarity)
        
        # Ridge continuity contribution
        continuity_score = min(1.0, ridge_continuity)
        
        # Standard deviation contribution (lower variation = more consistent = better visual organization)
        if std_intensity > 0:
            std_score = max(0.0, 1.0 - (std_intensity / 100.0))
        else:
            std_score = 0.5
        
        # Combine scores
        visual_organization = (regularity_score * 0.3 + lacunarity_score * 0.25 + continuity_score * 0.25 + std_score * 0.2)
        return min(1.0, visual_organization)
    
    def _calculate_spatial_reasoning(self, ridge_uniformity: float, pattern_type: str,
                                   ridge_curvature: float, spectral_energy: float) -> float:
        """Calculate spatial reasoning from fingerprint features (DMIT principle)"""
        # DMIT research shows: High ridge uniformity + pattern type = spatial reasoning
        
        # Ridge uniformity contribution
        uniformity_score = min(1.0, ridge_uniformity)
        
        # Pattern type contribution
        pattern_weights = {
            'whorl': 0.8,      # Complex patterns indicate spatial reasoning
            'loop': 0.7,       # Good spatial reasoning
            'arch': 0.6,       # Moderate spatial reasoning
            'tented_arch': 0.65, # Slightly better than plain arch
            'composite': 0.75   # Complex spatial reasoning patterns
        }
        pattern_score = pattern_weights.get(pattern_type.lower(), 0.6)
        
        # Ridge curvature contribution
        curvature_score = min(1.0, ridge_curvature)
        
        # Spectral energy contribution
        energy_score = min(1.0, spectral_energy)
        
        # Combine scores
        spatial_reasoning = (uniformity_score * 0.3 + pattern_score * 0.25 + curvature_score * 0.25 + energy_score * 0.2)
        return min(1.0, spatial_reasoning)
    
    def _calculate_visual_adaptability(self, spectral_centroid: float, spectral_rolloff: float,
                                     graph_density: float, topological_complexity: float) -> float:
        """Calculate visual adaptability from fingerprint features (DMIT principle)"""
        # DMIT research shows: High spectral features + graph complexity = visual adaptability
        
        # Spectral centroid contribution
        centroid_score = min(1.0, spectral_centroid)
        
        # Spectral rolloff contribution
        rolloff_score = min(1.0, spectral_rolloff)
        
        # Graph density contribution
        density_score = min(1.0, graph_density)
        
        # Topological complexity contribution
        complexity_score = min(1.0, topological_complexity)
        
        # Combine scores
        visual_adaptability = (centroid_score * 0.3 + rolloff_score * 0.25 + density_score * 0.25 + complexity_score * 0.2)
        return min(1.0, visual_adaptability)

    @staticmethod
    def classify_spatial_intelligence_level(score: float) -> str:
        if score >= 0.85:
            return "Exceptional Spatial Intelligence"
        elif score >= 0.75:
            return "High Spatial Intelligence"
        elif score >= 0.65:
            return "Above Average Spatial Intelligence"
        elif score >= 0.55:
            return "Average Spatial Intelligence"
        else:
            return "Developing Spatial Intelligence" 