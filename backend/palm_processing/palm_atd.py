...
class PalmAtdEstimator:
    ...
    @staticmethod
    def _wrist_point(points: np.ndarray, cx: int, y_min: int, height: int) -> Point:
        lower = points[points[:, 1] > y_min + 0.8 * height]
        if len(lower) == 0:
            lower = points[points[:, 1] > y_min + 0.6 * height]
        if len(lower) == 0:
            return (cx, int(points[:, 1].max()))
        wrist_x = int(np.mean(lower[:, 0]))
        wrist_y = int(np.max(lower[:, 1]))
