...
it segments the hand, locates the inter-finger webs and the wrist axis, and
computes the angle at the axial point t between the index-base (a) and
little-base (d) landmarks. Every result carries a confidence; below threshold the
estimate is withheld (returned as None) rather than guessed.
"""
...
class PalmAtdEstimator:
    ...
    # not real anatomy. Clinical ATD angles range ~25–62°; true values above
    # 62° are anatomically implausible from a casual photograph.
    ...
    def _axial_triradius_point(points: np.ndarray, cx: int, y_min: int, height: int) -> Point:
        """
        Approximate the position of the axial triradius 't'.

        In real dermatoglyphics, the 't' triradius sits at the proximal palm —
        just above the wrist crease, roughly 78–92% of the way down the hand
        bounding box from the fingertips.

        We use the centroid (not the max-y) of contour points in this band.
        Using max-y is unstable and picks stray pixels; centroid is robust.
        We also constrain the band to 78–92% so forearm pixels (outside the
        palm bounding box) are excluded.
        """
        y_low  = y_min + 0.78 * height
        y_high = y_min + 0.92 * height
        band = points[(points[:, 1] >= y_low) & (points[:, 1] <= y_high)]
        if len(band) < 2:
            # Fallback: fixed 85% from top
    ...
    def _atd_landmarks(self, contour, hand: str) -> Optional[Dict[str, Any]]:
        ...
        t = self._wrist_point(points, cx, y_min, height)
